    ...
    private static function update_entry($result, $entry, $config){
        ...
        if($result["is_success"]){
            ...
            if($config["meta"]["type"] == "product"){
                ...
                $fee_amount = self::$transaction_response["setup_fee"];
                if(!empty($fee_amount) && $fee_amount > 0){
                    RGFormsModel::add_note($entry["id"], 0, "System", sprintf(__("Setup fee payment has been made. Amount: %s. Subscription Id: %s", "gravityforms"), GFCommon::to_money($fee_amount, $entry["currency"]),$entry["transaction_id"]));
                    GFAuthorizeNetData::insert_transaction($entry["id"], "payment", $entry["transaction_id"], $entry["transaction_id"], $fee_amount);
                }
