...
class GFPayPalPaymentsPro {
    ...
    public static function settings_page(){
        ...
        <form action="" method="post">
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td width="80%">
                        <input type="text" class="size-1" id="gf_paypalpaymentspro_password" name="gf_paypalpaymentspro_password" value="<?php echo esc_attr($settings["password"]) ?>" />
                        <img src="<?php echo self::get_base_url() ?>/images/<?php echo $is_valid ? "tick.png" : "stop.png" ?>" border="0" alt="<?php $message ?>" title="<?php echo $message ?>" style="display:<?php echo empty($message) ? 'none;' : 'inline;' ?>" />
                    </td>
                </tr>
        ...
        <form action="" method="post">
            ...
            <?php if(GFCommon::current_user_can_any("gravityforms_paypalpaymentspro_uninstall")){ ?>
                ...
                <div class="delete-alert"><?php _e("Warning! This operation deletes ALL PayPal Payments Pro Feeds.", "gravityformspaypalpaymentspro") ?>
                    <?php
                    $uninstall_button = '<input type="submit" name="uninstall" value="' . __("Uninstall PayPal Payments Pro Add-On", "gravityformspaypalpaymentspro") . '" class="button" onclick="return confirm(\'' . __("Warning! ALL PayPal Payments Pro Feeds will be deleted. This cannot be undone. \'OK\' to delete, \'Cancel\' to stop", "gravityformspaypalpaymentspro") . '\');"/>';
                    echo apply_filters("gform_paypalpaymentspro_uninstall_button", $uninstall_button);
                    ?>
                </div>
            <?php } ?>
        </form>
    ...
    private static function get_local_api_settings($config){
        $local_api_settings = array("mode" => $config["meta"]["api_mode"], "username" => $config["meta"]["api_username"], "password" =>  $config["meta"]["api_password"]);
            ...
            ?>>
                ...
                <div id="paypalpaymentspro_field_container_subscription" class="paypalpaymentspro_field_container" valign="top" <?php echo rgars($config, "meta/type") != "subscription" ? "style='display:none;'" : ""?>>
                    ...
                    <div class="margin_vertical_10">
                        ...
                        <select id="gf_paypalpaymentspro_recurring_times" name="gf_paypalpaymentspro_recurring_times">
                            <option value=""><?php _e("Infinite", "gravityformspaypalpaymentspro") ?></option>
                            <?php
                            for($i=2; $i<=30; $i++){
                ...
                <div id="paypalpaymentspro_api_settings_container" <?php echo rgars($config, "meta/api_settings_enabled") ? "" : "style='display:none;'" ?>>
                    ...
                    <div class="margin_vertical_10">
                        ...
                        <input class="size-1" id="gf_paypalpaymentspro_api_password" name="gf_paypalpaymentspro_api_password" value="<?php echo rgar($config["meta"],"api_password") ?>" />
                        <img src="<?php echo self::get_base_url() ?>/images/<?php echo rgars($config, "meta/api_valid") ? "tick.png" : "stop.png" ?>" border="0" alt="<?php echo $config["meta"]["api_message"] ?>" title="<?php echo $config["meta"]["api_message"] ?>" style="display:<?php echo empty($config["meta"]["api_message"]) ? 'none;' : 'inline;' ?>" />
                    </div>
    ...
    public static function paypalpaymentspro_validation($validation_result){
        ...
        if(strlen($expiration_date[0]) == 1) $expiration_date[0] = '0'.$expiration_date[0];
            $billing['EXPDATE'] = $expiration_date[0].substr($expiration_date[1], -2);
        ...
        $billing['BILLTOCOUNTRY'] = $country == "UK" ? "GB" : $country;
        $billing['CURRENCY'] = GFCommon::get_currency();
        ...
        if($config["meta"]["type"] == "product" && $amount == 0){
        ...
        self::log_debug("Authorizing credit card information for later capture.");
        $billing = apply_filters("gform_paypalpaymentspro_args_before_authorize", $billing, $lead["form_id"]);

        if(!empty($local_api_settings))
            $response = self::post_to_payflow($billing, $local_api_settings, $lead["form_id"]);
        else
            $response = self::post_to_payflow($billing, array(), $lead["form_id"]);

        if(!empty($response) && $response["RESULT"] == "0" && !empty($response["PNREF"])){

            if($config["meta"]["type"] == "product"){
                self::$transaction_response = array("transaction_id" => $response["PNREF"], "subscription_amount" => 0, "amount" => $amount, "transaction_type" => 1, 'config_id' => $config['id']);
                self::log_debug("Authorization for one time payment successful.");
                return $validation_result;
            }
            else
            {
                //setting up recurring transaction parameters for later subscription creation
                $billing['TENDER'] = "C";
                $billing['TRXTYPE'] = "R";
                $billing['ACTION'] = "A";
                ...
                self::$transaction_response = array("transaction_id" => rgar($response,"PNREF"),  "subscription_amount" => $billing['AMT'], "initial_payment_amount" => $setup_fee_amount, "transaction_type" => 2, 'config_id' => $config['id'] );
                self::log_debug("Recurring payment authorization successful.");
                return $validation_result;
            }
        }
        else
        {
            // Authorization was not succesful, need to display error message
            self::log_error("Payment Authorization was NOT successful.");
                    ...
                    switch(strtolower($status)){
                        case "active" :
                            ...
                            if($new_payment_date > $querydate)
                            {
                                ...
                                GFPayPalPaymentsProData::insert_transaction($entry["id"],"payment", $subscription_id, $transaction_id, $profile_status["AMT"]);
    ...
    public static function paypalpaymentspro_commit_transaction($entry,$form){
        ...
        if($transaction_type == "2")
        {
            ...
            //Create subscription and capture initial payment if needed
            self::capture_subscription_payment($entry,$transaction_id,$fee_amount,$config);

        }
        else
        {
            if(self::capture_product_payment($entry,$config))
                GFPayPalPaymentsProData::insert_transaction($entry["id"], "payment", $subscriber_id, self::$transaction_response["transaction_id"], $amount);
    ...
    private static function capture_product_payment($entry,$config){

        $origid = self::$transaction_response["transaction_id"];

        $args = array("TRXTYPE" => "D", "TENDER" => "C", "ORIGID" => $origid);
        ...
        $args = apply_filters("gform_paypalpaymentspro_args_before_payment", $args, $entry["form_id"]);

        $response = self::post_to_payflow($args,$local_api_settings, $entry["form_id"]);

        if($response["RESULT"] == 0)
        {
            $entry["transaction_id"] = $response["PNREF"];
            $entry["payment_status"] = "Approved";
            RGFormsModel::update_lead($entry);
            self::$transaction_response["transaction_id"] = $response["PNREF"];
            return true;
        }
        else
            return false;
    ...
    private static function capture_subscription_payment($entry,$transaction_id,$fee_amount,$config){
        ...
        $response = self::post_to_payflow($subscription,$local_api_settings, $entry["form_id"]);
        ...
        if($response["RESULT"] == 0)
        {
            ...
            $entry["transaction_id"] = $response["PROFILEID"];
            $entry["payment_date"] = date("Y-m-d H:i:s", $tomorrow);
            RGFormsModel::update_lead($entry);

            //Add subsciption creation and initial payment note
            RGFormsModel::add_note($entry["id"], 0, "System", sprintf(__("Subscription has been created and initial payment will be made %s. Amount: %s. Subscriber Id: %s", "gravityforms"), date("m-d-Y", $tomorrow), GFCommon::to_money($subscription["AMT"], $entry["currency"]),$response["PROFILEID"]));
            GFPayPalPaymentsProData::insert_transaction($entry["id"], "payment", $response["PROFILEID"], $response["PROFILEID"], $subscription["AMT"]);

            //Add note for setup fee payment if completed
            if(!empty($fee_amount) && $fee_amount > 0){
                RGFormsModel::add_note($entry["id"], 0, "System", sprintf(__("Setup fee payment has been made. Amount: %s. Subscriber Id: %s", "gravityforms"), GFCommon::to_money($fee_amount, $entry["currency"]),$response["PROFILEID"]));
                GFPayPalPaymentsProData::insert_transaction($entry["id"], "payment", $response["PROFILEID"], $response["PROFILEID"], $fee_amount);
            }
