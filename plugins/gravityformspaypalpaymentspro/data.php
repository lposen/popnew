<?php
class GFPayPalPaymentsProData{

    public static function update_table(){
        global $wpdb;
        $table_name = self::get_paypalpaymentspro_table_name();
    ...
    public static function get_transaction_totals($config){
        ...
        $sql = $wpdb->prepare("SELECT sum(t.amount) revenue, count({$orders}) as orders
                                 ...
                                 INNER JOIN {$wpdb->prefix}rg_lead_meta m ON meta_key='paypalpaymentspro_feed_id' AND m.lead_id = l.id
    ...
    public static function get_paypalpaymentspro_table_name(){
        global $wpdb;
        return $wpdb->prefix . "rg_paypalpaymentspro";
    }
    ...
    public static function drop_tables(){
        global $wpdb;
        $wpdb->query("DROP TABLE IF EXISTS " . self::get_paypalpaymentspro_table_name());
    }
