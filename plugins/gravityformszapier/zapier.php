...
class GFZapier {
	...
	private static function zapier_list_page($form_id){
        ...
        <?php
        $zapier_table = new GFZapierTable();
        $zapier_table->prepare_items($form_id);
        ?>
	...
	public static function send_form_data_to_zapier($entry = null, $form){
		...
		foreach($form["fields"] as $field){
			if (!empty($field["inputs"])){
				//field has inputs, complex field like name or address, get individual settings/data
				foreach($field["inputs"] as $input){
					$label = GFFormsModel::get_label($field,$input["id"]);
					$field_id = (string) $input["id"];
					$field_data = $is_entry ? $entry[$field_id] : "";
					$body[$label] = $field_data;
				}
			}
			else{
				$label = GFFormsModel::get_label($field,$field["id"]);
...
class GFZapierTable extends WP_List_Table {
    ...
    function prepare_items($form_id) {
    	//query db
 		$zaps = GFZapierData::get_feed_by_form($form_id);
