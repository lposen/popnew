...
class GFPayPalPro {
    ...
    public static function settings_page(){
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <ul>
                            ...
                            <li><?php echo sprintf(__("Click the box to enable IPN and enter the following Notification URL: %s", "gravityformspaypal"), "<strong>" . add_query_arg("page", "gf_paypalpro_ipn", get_bloginfo("url") . "/") . "</strong>") ?></li>
                        </ul>
                    </td>
                </tr>
        ...
        <form action="" method="post">
            ...
            <?php if(GFCommon::current_user_can_any("gravityforms_paypalpro_uninstall")){ ?>
                ...
                <h3><?php _e("Uninstall PayPal Pro Add-On", "gravityformspaypalpro") ?></h3>
                <div class="delete-alert"><?php _e("Warning! This operation deletes ALL PayPal Pro Feeds.", "gravityformspaypalpro") ?>
                    <?php
                    $uninstall_button = '<input type="submit" name="uninstall" value="' . __("Uninstall PayPal Pro Add-On", "gravityformspaypalpro") . '" class="button" onclick="return confirm(\'' . __("Warning! ALL PayPal Pro Feeds will be deleted. This cannot be undone. \'OK\' to delete, \'Cancel\' to stop", "gravityformspaypalpro") . '\');"/>';
                    echo apply_filters("gform_paypalpro_uninstall_button", $uninstall_button);
                    ?>
                </div>
            <?php } ?>
        </form>
    ...
    public static function send_request($config, $fields, &$response=array(), $form, $entry){
        ...
        $body = http_build_query($login) . "&" . $fields;
    ...
    private static function verify_paypalpro_ipn(){
        ...
        //Post back to PayPal Pro system to validate
        $request = new WP_Http();
