			...
			<?php
				...
				if($upcoming_total > 0) : 
					while($upcoming->fetchRecord()) :
						$name = $upcoming->display('name');
						$intro = $upcoming->display('event_intro');
						$thumb = $upcoming->display('thumb_url');
						$thumb_src = wp_get_attachment_image_src($thumb[0]['ID']);
						$purchase = $upcoming->display('purchase_url');
						$raw_date = strtotime($upcoming->display('date'));
						$date = date('m/d/o', strtotime($upcoming->display('date')));
						$type = $upcoming->display('event_type');
