	...
	<?php
		...
		if($links_total > 0) :
			...
			while($links->fetchRecord()) :
				...
				$linkText = $links->display('link_text');
                                $linkCustomCode = $links->display('link_custom_code');
                                $price = $links->display('price');
                                $page = $links->display('page');
                                $desc = $links->display('description');
				$image_raw = $links->display('photo');
                                $image = $image_raw[0]['guid'];
				$closeup_raw = $links->display('photo_large');                            
                                $closeup = $image;
                                if (!$linkText) {
                                    $linkText="Buy Now";
                                }
