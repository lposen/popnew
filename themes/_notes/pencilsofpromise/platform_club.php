...
<link rel="stylesheet" href="<?php bloginfo('template_directory'); ?>/platform_style.css" type="text/css" media="screen" />

<?php
//-------------------BEGIN PLATFORM DISPLAY PAGE INIT----------------------//
...
$loginId=get_user_meta($current_user->ID, 'userSalesforce', true);
if (!$loginId) {
...
$fundId = $_GET["f"];
$contactId=$_GET["u"];
//-------------------END PLATFORM DISPLAY PAGE INIT------------------------//
?>
...
<div id="main">
    ...
    <?php
        ...
        $response = $mySforceConnection->query($query);
         foreach ($response->records as $record) {
            echo '<li><a href="/fundraise/fundraiser?f='.$record->Id.'">'.$record->fields->Name.'</a></li>';
        };       
    ?>
    </ul>
    </div> 
    ...
    <div class="platformsection">
            ...
            <?php
                ...
                $response = $mySforceConnection->query($query);
                 foreach ($response->records as $record) {
                    echo '<li><a href="/userprofile?u='.$record->Id.'">'.$record->fields->Name.'</a></li>';
                };       
            ?>
            </ul>
    </div>    
