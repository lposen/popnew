...
<div id="clubSection"> 
    ...
    <?php 
    $adminClubDetailId;
    $query = "SELECT id,Name FROM Group__c WHERE Published__c = TRUE AND id IN (SELECT Group__c FROM Group_Admin__c WHERE Contact__c = '".$loginId."') LIMIT 100";
    $response = $mySforceConnection->query($query);
    foreach ($response->records as $record) {
    ...
    <ul>
    <?php  
    $query = "SELECT id,Name FROM Group__c WHERE Published__c = TRUE AND id NOT IN (SELECT Group__c FROM Group_Admin__c WHERE Contact__c = '".$loginId."') LIMIT 100";
    $response = $mySforceConnection->query($query);
    foreach ($response->records as $record) {
        echo '<li>'.$record->fields->Name.'</li>';
    }; 
...
<div id="clubSection"> 
    ...
    if ($groupId!='') {
        ?>
            <p>Showing details for club id: <?php echo $groupId; ?></p>
        <?php
    }
    else {
        $groupId = $adminClubDetailId;
        ?>
            <p>Showing first admin club for this user with id: <?php echo $groupId; ?></p>
        <?php    
    }
    $query = "SELECT Name, Description__c, Location__c FROM Group__c WHERE Id = '".$groupId."' AND Published__c = TRUE LIMIT 1";
    $response = $mySforceConnection->query($query);
    foreach ($response->records as $record) {
...
<div id="clubSection">
    ...
    <?php
        $fundraiserId;
        $query = "SELECT id,Name FROM Fundraiser__c WHERE id IN (SELECT Fundraiser__c FROM Group_Fundraiser__c WHERE Group__c = '".$groupId."') LIMIT 100";
        $response = $mySforceConnection->query($query);
         foreach ($response->records as $record) {
...
<div id="clubSection">
    ...
    <ul>
    <?php 
        $query = "SELECT id,Name FROM Contact WHERE id IN (SELECT Contact__c FROM Group_Admin__c WHERE Group__c = '".$groupId."') LIMIT 100";
        $response = $mySforceConnection->query($query);
         foreach ($response->records as $record) {
