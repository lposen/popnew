...
<div id="introduction">
	...
	foreach($uIDs as $uID) { 
		...
		if ($theirTitle == "volunteer") {
			array_push($volunteers, $uID);
		}
	}
	
	$max = (count($volunteers) < 5) ? count($volunteers) : 5;
	?>
	...
	<?php
		...
		foreach($authors_arr as $author) {
			...
			if ($user_title != "hidden" && $user_title == "board" && $user_id != $adamID) {
				...
				echo '<div id="tooltip'.$i.'" class="tooltipwordyleft">'.$user_desc.'</div>';
                            }
                            else {
                                echo '<div id="tooltip'.$i.'" class="tooltipwordy">'.$user_desc.'</div>';
                            }
                                echo '<div class="member">
					<div class="avatar medium" id="bod'.$i.'">' . $user_avatar . '</div>
					<h3>' . $user_full_name . '</h3>
				</div>';
                        if ($user_desc != "") {
                            if ($i==4 || $i==5 || $i==6 || $i==10 || $i==11 || $i==12) {
                                $script.='$("#bod'.$i.'").tooltip({tip: \'#tooltip'.$i.'\',position: \'center left\',offset: [-10, 9],delay: 0});';
                            }
                            else {
                                $script.='$("#bod'.$i.'").tooltip({tip: \'#tooltip'.$i.'\',position: \'center right\',offset: [-10,-9],delay: 0});';
                            }
			...
			if ($user_title != "hidden" && $user_title == "advisory" && $user_id != $adamID) {
				echo '<div id="tooltip'.$i.'" class="tooltipwordy">'.$user_desc.'</div>';
					...
					<h3>' . $user_full_name . '</h3>
                        ...
                        if ($user_desc != "") {        
                        $script.='$("#bod'.$i.'").tooltip({tip: \'#tooltip'.$i.'\',position: \'center right\',offset: [-10,-30],delay: 0});';
                        }
                        $i=$i+1;        
			}
		}
