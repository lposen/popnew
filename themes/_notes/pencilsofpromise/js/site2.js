...
$(function() {
	...
	$(".rightside.name").next().remove();
	$(".rightside.dobmm").next().remove();
	$(".rightside.dobdd").next().remove();
	
	$('<hr />').insertBefore(".leftside.name").prev();
	$('<hr />').insertBefore(".rightside.dobmm").prev();
	$('<hr />').insertAfter(".rightside.gender").next().next();
	
	$(".rightside.optin").detach().insertBefore(".leftside.optin");
	$('#pods_form1_optin').attr('checked', true);
	
	$('<p class="error"></p>').insertAfter(".leftside.optin").next().next();
	
	$("<span> ... </span>").insertBefore("a.lastPage");
	
	$('<img src="/wp-content/themes/pencilsofpromise/gfx/season1000/txtYourDateOfBirth.gif" alt="Your Date of Birth" class="sLabel" />').insertBefore(".rightside.dobmm").prev();
	
	if ($.cookie("season1000") == "true") {
		$("#sThanks").show();
		$.cookie("season1000", null);
		
		$("#sThanksYes").click(function(){
			var message = "I+just+changed+the+world+with+@pencilsofpromis.+Make+your+%23Promise+now!+%23seasonofpromises";
			window.open('http://twitter.com/share?url=http://www.pencilsofpromise.org/promises&text='+message, 'Update', "status=0, width=600, height=300, left=400, top=100");
			//$("#sThanks").slideUp();
			try {
				_gaq.push(['_trackEvent', 'Season of 1000 Promises', 'Tweet']);
			} catch (err) {}
			window.location = "https://www.pencilsofpromise.org/join-the-movement/donate/?recurring=monthly";
		});
		
		$("#sThanksNo").click(function(){
			//$("#sThanks").slideUp();
			try {
				_gaq.push(['_trackEvent', 'Season of 1000 Promises', 'Bypass Tweet']);
			} catch (err) {}
			window.location = "https://www.pencilsofpromise.org/join-the-movement/donate/?recurring=monthly";
		});
	}
	...
	jQuery("#season1000 input[type='text'], #season1000 textarea").cleardefault();
	
	$('#season1000 .btn_save').attr('onclick','').unbind('click');
	
	$('#season1000 .btn_save').click(function(){
		var hasErrors = false;
		
		if ($('#pods_form1_name').val() == "Your First Name *") {
			hasErrors = true;
		}
		if ($('#pods_form1_email').val() == "Your Email *") {
			hasErrors = true;
		}
		if ($('#pods_form1_promise').val() == "Make Your Promise Here *") {
			hasErrors = true;
		}
		
		if (!hasErrors) {
			$('#season1000 p.error').text("All set! Hold on while we work some magic!");
			$('#season1000 .btn_save').hide();
			$('#season1000 .leftside.optin').hide();
			$('#season1000 .rightside.optin').hide();
			$.cookie("season1000", "true");
			if ($('#pods_form1_zipcode').val() == "Your Zipcode") {
				$('#pods_form1_zipcode').val("");
			}
			if ($('#pods_form1_lname').val() == "Your Last Name") {
				$('#pods_form1_lname').val("");
			}
			saveForm(1);
			try {
				_gaq.push(['_trackEvent', 'Season of 1000 Promises', 'Submit a Promise']);
			} catch (err) {}
		} else {
			$('#season1000 p.error').text("Uh oh! Please make sure you've filled in your first name, e-mail address and your promise into the fields above before continuing.");
		}
	});
	...
	$("#featurebox li.default .show51").hover(function() {
	    	$(".hometouts .fiftyone").css("background-position", "0 -187px");
			$("#featurebox li.default .img0").stop(true, true).fadeOut(200).next().fadeIn(200);
	  	},
		function() {
		    $(".hometouts .fiftyone").css("background-position", "0 0").removeAttr('style');
			$("#featurebox li.default .img51").stop(true, true).fadeOut(200).prev().fadeIn(200);
	});
	
	$("#featurebox li.default .show49").hover(function() {
	    	$(".hometouts .fourtynine").css("background-position", "0 -187px");
			$("#featurebox li.default .img0").stop(true, true).fadeOut(200).next().next().fadeIn(200);
	  	},
		function() {
		    $(".hometouts .fourtynine").css("background-position", "0 0").removeAttr('style');
			$("#featurebox li.default .img49").stop(true, true).fadeOut(200).prev().prev().fadeIn(200);
	});
    ...
    $(document).ready(function(){
            if ($.browser.msie) {
              ...
              $("#ipromise #stats").hide();
                    ...
                    function(data) {
                         $('#stats #amount-raised').append("$" + addCommas(Math.round(data['total_raised'])));
                         $('#stats #num-donors').append(addCommas(Math.round(data['total_supporters'])));
                         $('#stats #num-fundraisers').append(addCommas(Math.round(data['total_fund_pages'])));
                         $('#stats #num-teams').append(Math.round(data['total_fund_teams']));
                    });
                    ...
                    function(data) {
                            ...
                            $('#ipromise-stats #board .boardTable .boardTableTable').append('<thead><tr><td colspan="4"><h2>TOP FUNDRAISERS</h2></td></tr></thead>');
                            $('#ipromise-stats #board .boardTable .boardTableTable').append('<tbody></tbody>');
                            $.each(data, function(i,item){
                                $('#ipromise-stats #board .boardTable .boardTableTable tbody').append('<tr><td class="big heavy">'+(i+1)+'.</td><td><img src="'+item['member_pic']+'" width="30" height="30"/></td><td class="name">'+item['member_name']+'</td><td class="middle">$'+addCommas(Math.round(item['total_raised']))+'</td></tr>');
                            });
                    });                 
            }      
    });
